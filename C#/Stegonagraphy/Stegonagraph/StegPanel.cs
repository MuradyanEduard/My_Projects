    ...
    public partial class StegPanel : Form
    {
                ...
                foreach (String file in openFileDialog1.FileNames)
                {
                    ...
                    switch (fileInfo.Extension.ToLower())
                    {
                        ...
                        case ".jpeg":
                            ...
                            pbPicture.Image = Image.FromFile("Image/true.png");
                            containerGridView.Rows.Add(fileInfo.Name, jpeg.GetInfo(stegKey), fileInfo.FullName);
                            break;
                        default:
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            if (tpHide)
            {
                ...
                String hideFileCount = HelpTools.AutoAddByte(Convert.ToString((Int64)GetSize(dataGridView), 2), 40);

                //amboxj informatian byterov
                for (int i = 0; i < 5; i++)
                    hideInfo.Add(Convert.ToByte(hideFileCount.Substring(i * 8, 8), 2));

                hideFileCount = HelpTools.AutoAddByte(Convert.ToString(pathArray.Length, 2), 40);
                //fileri qanaky
                for (int i = 0; i < 5; i++)
                    hideInfo.Add(Convert.ToByte(hideFileCount.Substring(i * 8, 8), 2));
                ...
                for (int i = 0; i < pathArray.Length; i++)
                {
                    ...
                    String fileCountStr = HelpTools.AutoAddByte(Convert.ToString(btHideArray.Length, 2), 40);

                    for (int j = 0; j < 5; j++)
                        hideInfo.Add(Convert.ToByte(fileCountStr.Substring(j * 8, 8), 2));
                ...
                for (int i = 0; i < containerGridView.Rows.Count; i++)
                {
                    ...
                    switch (fileInfo.Extension.ToLower())
                    {
                        ...
                        case ".jpeg":
                            ...
                            findInfo.AddRange(jpeg.jpegDecode(stegKey));
                            break;
                    }
                }
