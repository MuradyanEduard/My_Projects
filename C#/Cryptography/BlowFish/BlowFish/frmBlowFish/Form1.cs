...
namespace frmBlowFish
{
    ...
    {
        static UInt32[] Sbox0 = {0xd1310ba6, 0x98dfb5ac, 0x2ffd72db, 0xd01adfb7, 0xb8e1afed, 0x6a267e96,
        ...
        static UInt32[] FIXED_P = {
    0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0,
    0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c,
    0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917, 0x9216d5d9, 0x8979fb1b,
};
        static UInt32[] EXT_KEY = {
    0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0,
    0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c,
    0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917, 0x9216d5d9, 0x8979fb1b,
};
        ...
        private void BtnStart_Click(object sender, EventArgs e)
        {
            ...
            if (radioButtonE.Checked == false && radioButtonD.Checked == false)
            {
                MessageBox.Show("Select method(Encrypt/Decrypt)!");
                return;
            }

            if (text == "")
            {
                MessageBox.Show("Input the text!");
                return;
            }

            if (key == "")
            {
                MessageBox.Show("Input the key!");
                return;
            }
            ...
            for (int i = 0; i < Text8.Length; i++)
                str += Convert.ToChar(Text8[i]).ToString();
        ...
        static private byte[] Decrypt(byte[] chiperText)
        {
            ...
            for (int i = 0; i < block32.Length / 2; i++)
            {
                ...
                block32[2 * i] = block32[2 * i] ^ EXT_KEY[0];
                block32[2 * i + 1] = block32[2 * i + 1] ^ EXT_KEY[1];
        ...
        static private byte[] Encrypt(byte[] plainText)
        {
            ...
            for (int i = 0; i < block32.Length / 2; i++)
            {
                ...
                block32[2 * i] = block32[2 * i] ^ EXT_KEY[17];
                block32[2 * i + 1] = block32[2 * i + 1] ^ EXT_KEY[16];
            }
            return F32To8Block(block32);
        }
        ...
        static private void keyExtension(String bfKey)
        {
            ...
            for (int i = 0; i < FIXED_P.Length; i++)
                EXT_KEY[i] = FIXED_P[i] ^ key32Block[i];
        ...
        static private UInt32[] FeistelNetwork(UInt32 leftPiece, UInt32 rightPiece, int index)
        {
            ...
            leftPiece = leftPiece ^ EXT_KEY[index];
        ...
        static private UInt32 FuncF(UInt32 sboxText)
        {
            byte[] block8;
            UInt32[] block32 = new UInt32[4];
            UInt32 funcEnd;

            block8 = F32To8Block(new UInt32[1] { sboxText });
